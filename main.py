...
def run_deseq2(
    ...
    if counts_df1.shape[1] < 10:
        raise HTTPException(
            status_code=400,
            detail=f"Too few informative genes after filtering ({counts_df1.shape[1]}). "
    ...
    try:
        ...
        if counts_df2.shape[1] < 10:
            raise HTTPException(
                status_code=400,
                detail="DE failed even after stricter filtering. "
...
@app.post("/diffexp", response_model=DEResponse)
